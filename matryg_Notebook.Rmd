...
plot(train_model1)
```
From the above linear model based on multiple variables the main point for now is to look at the residual standard error, which is 1.08 and the R-squared is 0.8444 which is approx 84 %. This will be discussed in a moment after having trained the last linear model with multiple variables which will be age, height in meter, smoke, gender and caesarean.
```{r}
...
plot(train_model2)
```
For the last model with multiple variables the residual standard error is 1.04 and the R-squared is 0.8565 which is approx. 86 %. Compared with the other two models based on the training set, this model has the lowest error and highest percentage of R-squared which means that it explains 86 % of the dependent variable variation. Then follows the other model with multiple variables (train_model1) and at last comes the model with a single parameter.  
From now on I will refer to the first model - the one with height in meter as x-variable, as model 1. I will refer to the second model - the one with height in meter and age as x-variables, as model 2. And I will refer to the last model - the one with all possible x-variables, as model 3. Please note that the models will not be renamed in the code, but model 1 = mod, model 2 = model1 and model 3 = model2.  
...
```{r}
# model 1
lungCap_mod_rmse_train <- training_set %>%
   mutate(predRegModTrain = predict(train_mod))
# model 2
lungCap_mod_rmse_train$predRegModel1Train = predict(train_model1)
# model 3
lungCap_mod_rmse_train$predRegModel2Train = predict(train_model2)
```
The I will look at the actual values plotted against the predicted values.
```{r}
plot(lungCap_mod_rmse_train$LungCap, lungCap_mod_rmse_train$predRegModTrain, las=1, main="Scatterplot - actual values vs. predicted values\n - model 1", ylab="Predicted value", xlab="Actual value")

plot(lungCap_mod_rmse_train$LungCap, lungCap_mod_rmse_train$predRegModel1Train, las=1, main="Scatterplot - actual values vs. predicted values\n - model 2", ylab="Predicted value", xlab="Actual value")

plot(lungCap_mod_rmse_train$LungCap, lungCap_mod_rmse_train$predRegModel2Train, las=1, main="Scatterplot - actual values vs. predicted values\n - model 3", ylab="Predicted value", xlab="Actual value")
```
All of the models shows a positive linear relationship between the actual values and predicted values. There don't seems to be any outliers. The correlation shows that when an actual value increase so does the predicted one. When comparing the three scatterplots it is possible to observe that models with more X-variables is more dense than models with a lesser number of x-variables.  

Now I will create the mean squared error for each model.
```{r}
# model 1
mse_mod <- lungCap_mod_rmse_train %>% mutate(errorMod = predRegModTrain - LungCap, sqrErrorMod = errorMod^2) %>% summarise(mse_mod = mean(sqrErrorMod))
# model 2
mse_model1 <- lungCap_mod_rmse_train %>% mutate(errorModel1 = predRegModel1Train - LungCap, sqrErrorModel1 = errorModel1^2) %>% summarise(mse_model1 = mean(sqrErrorModel1))
# model 3
mse_model2 <- lungCap_mod_rmse_train %>% mutate(errorModel2 = predRegModel2Train - LungCap, sqrErrorModel2 = errorModel2^2) %>% summarise(mse_model2 = mean(sqrErrorModel2))

print(paste0("Mean squared error for model 1: ",mse_mod))
print(paste0("Mean squared error for model 2: ",mse_model1))
print(paste0("Mean squared error for model 3: ",mse_model2))
```
...
```{r}
# Model 1
RMSE_model1 <- sqrt(mse_mod)
print(paste0("RMSE for model 1: ", RMSE_model1))
# Model 1
RMSE_model2 <- sqrt(mse_model1)
print(paste0("RMSE for model 2: ", RMSE_model2))
# Model 1
RMSE_model3 <- sqrt(mse_model2)
print(paste0("RMSE for model 3: ", RMSE_model3))

```
All RMSE values shows a pretty fine average error which is just above approx. 7 %. This RMSE value was for the training set, so I will also have to look into the testing set. Remember that the models have not seen the data in the test set and if the model is good I should get a RMSE value for each value that is close to the RMSE value from the training set.
